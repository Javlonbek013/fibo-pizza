...
const Soup_list = ({ addToCard, addToFavorites }) => {
  ...
  return (
    <div className="container">
      <h1 className="text-yellov">Супы</h1>
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 pt-[30px]">
        {soups.map((item) => (
          <div key={item.id} className="relative rounded-2xl p-8 bg-white">
            ...
            <div className="w-full h-38 bg-gray-200 rounded-lg mb-5 flex items-center justify-center">
              <img src={item.image} alt={item.name} className="object-fill rounded-lg" />
            </div>
